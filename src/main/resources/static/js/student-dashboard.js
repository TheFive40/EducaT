...
window.addEventListener('focus', () => {
    ...
    if (courseView && courseView.classList.contains('show') && currentCourse && currentCourse.id && currentStudent && currentStudent.id) {
        ...
        // No refrescar la unidad si el estudiante está en medio de una evaluación
        const isTakingExam = !!(examTakerState && examTakerState.attemptId);
        if (!anyEditing && !isTakingExam) {
            const openIds = getOpenStudentCardIds();
...
async function confirmExamAccessKey(examId) {
    ...
    if (key !== exam.accessKey) { showToast('Clave incorrecta', 'error'); return; }
    closeModal();
...
function openSebRequiredModal(examId) {
    ...
    document.getElementById('modalBody').innerHTML = `
        <div style="text-align:center;margin-bottom:16px">
            ...
            <div style="font-size:16px;font-weight:700;color:var(--navy)">Esta evaluación requiere Safe Exam Browser (SEB)</div>
        </div>
...
async function startStudentExam(examId) {
    ...
    if (exam.requireSeb || cfg.requireSeb) {
        if (!isSebBrowser()) {
            openSebRequiredModal(examId);
            return;
        }
    }
    if (exam.accessKey) {
        openExamAccessKeyModal(examId, exam);
        return;
    }
    await doStartStudentExam(exam);
}
