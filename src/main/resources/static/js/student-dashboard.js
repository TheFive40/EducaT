...
const MOCK = {
    user: {
        ...
        email: 'maria@educat.edu.co',
        role: {id: 3, name: 'ESTUDIANTE'},
        status: true,
    ...
    courses: [
        {
            ...
            description: 'Curso integral que abarca álgebra lineal, cálculo diferencial e integral con aplicaciones prácticas en ingeniería y ciencias.',
            teacher: {id: 1, specialization: 'Matemáticas', user: {id: 10, name: 'Dr. Carlos Martínez Lozano'}}
        },
        {
            id: 2,
            name: 'Literatura y Expresión',
            description: 'Análisis literario de textos hispanoamericanos, comprensión lectora avanzada y producción textual de diferentes géneros.',
            teacher: {id: 2, specialization: 'Letras Hispánicas', user: {id: 11, name: 'Dra. Ana Lucía Pérez Mora'}}
        },
        {
            id: 3,
            name: 'Ciencias Naturales',
            description: 'Estudio del entorno natural, fenómenos físicos y químicos con énfasis en metodología científica e investigación aplicada.',
            teacher: {id: 3, specialization: 'Biología y Química', user: {id: 12, name: 'Mg. Roberto Silva Arango'}}
        },
    ],
    enrollments: [
    ...
    grades: [
        {
            ...
            grade: 8.5,
            description: 'Excelente desempeño en álgebra lineal'
        },
        {
            id: 2,
            course: {id: 2, name: 'Literatura y Expresión'},
            grade: 7.2,
            description: 'Buen análisis crítico y argumentación'
        },
        {
            id: 3,
            course: {id: 3, name: 'Ciencias Naturales'},
            grade: 9.0,
            description: 'Sobresaliente en práctica de laboratorio'
        },
    ],
    attendance: [
    ...
    activities: [
        {
            ...
            description: 'Resolver los ejercicios de matrices 3x3 del capítulo 2 y presentar procedimiento completo.',
            dueDate: '2025-04-15'
        },
        {
            id: 2,
            ...
            description: 'Aplicación de reglas de derivación en funciones implícitas y paramétricas.',
            dueDate: '2025-04-22'
        },
        {
            id: 3,
            ...
            description: 'Análisis temático y estilístico de "Cien años de soledad" — mínimo 3 páginas.',
            dueDate: '2025-04-18'
        },
        {
            id: 4,
            ...
            description: 'Informe completo del experimento de reacciones ácido-base con conclusiones.',
            dueDate: '2025-04-20'
        },
    ],
    exams: [
    ...
    forums: [
        {
            ...
            description: 'Espacio para resolver dudas de la Unidad 1.'
        },
        {
            id: 2,
            courseId: 2,
            title: 'Debate: Realismo Mágico',
            description: 'Discusión sobre los elementos del realismo mágico en la narrativa latinoamericana.'
        },
        {
            id: 3,
            courseId: 3,
            title: 'Foro: Cambio Climático y Ciencia',
            description: 'Análisis científico del cambio climático y sus implicaciones.'
        },
    ],
};
    ...
    1: [
        {
            ...
            description: 'Esta unidad cubre vectores en espacios n-dimensionales, matrices y operaciones con matrices, transformaciones lineales, sistemas de ecuaciones lineales y su resolución mediante eliminación gaussiana.',
            announcements: ['Recuerden entregar el taller de matrices antes del viernes a las 11:59 p.m.', 'La próxima clase habrá un quiz corto de 10 minutos sobre operaciones matriciales.'],
            activities: [1],
            exams: [1],
            forums: [1],
            resources: [{
                name: 'Álgebra Lineal — Howard Anton (Cap. 1-3)',
                type: 'pdf',
                url: '#'
            }, {
                name: 'Video: Introducción a Matrices y Vectores',
                type: 'video',
                url: '#'
        ...
        },
        {
            ...
            welcome: 'Continuamos con el cálculo diferencial, una de las herramientas más poderosas de las matemáticas modernas.',
            description: 'Estudiaremos límites y su definición formal, continuidad de funciones, la derivada y sus interpretaciones geométrica y física, reglas de derivación, y aplicaciones de las derivadas en optimización.',
            announcements: ['Consultar el capítulo 2 de Stewart antes de la próxima clase.'],
            activities: [2],
            exams: [],
            forums: [],
            resources: [{
                name: 'Cálculo — James Stewart (Cap. 2-3)',
                type: 'pdf',
                url: '#'
    ...
    2: [
        {
            ...
            welcome: 'Bienvenidos a Literatura y Expresión. Nos adentraremos en el fascinante universo de la narrativa hispanoamericana.',
            description: 'Analizaremos las técnicas narrativas del realismo mágico, el contexto del "Boom" latinoamericano y la obra de García Márquez, Julio Cortázar y otros autores fundamentales.',
            announcements: ['Traer el libro "Cien años de soledad" a la próxima clase.', 'El ensayo literario debe subirse en formato PDF antes del 18 de abril.'],
            activities: [3],
            exams: [2],
            forums: [2],
            resources: [{
                name: 'García Márquez — Cien años de soledad',
                type: 'pdf',
                url: '#'
    ...
    3: [
        {
            ...
            welcome: 'Bienvenidos a Ciencias Naturales. Esta unidad establece los fundamentos de la química que necesitarán a lo largo del curso.',
            description: 'Abordaremos estructura atómica y la tabla periódica, tipos de enlace químico, reacciones químicas y balanceo, estequiometría básica, y ácidos y bases con aplicaciones prácticas de laboratorio.',
            announcements: ['El informe de laboratorio debe entregarse en formato impreso y digital.', 'Próxima práctica de laboratorio: traer bata blanca obligatoriamente.'],
            activities: [4],
            exams: [3],
            forums: [3],
            resources: [{
                name: 'Química — Chang y Goldsby (Cap. 1-4)',
                type: 'pdf',
                url: '#'
...
function getEmail() {
    return localStorage.getItem('educat_email') || sessionStorage.getItem('educat_email');
}
...
function getUnits(courseId) {
    ...
    return DEFAULT_UNITS[courseId] ? JSON.parse(JSON.stringify(DEFAULT_UNITS[courseId])) : [];
}
...
function navigateTo(section) {
    ...
    // Hide course view when navigating away
...
async function loadOverview() {
    ...
    enrollments = (enrData && enrData.length) ? enrData : MOCK.enrollments.map((e, i) => ({
        ...e,
        course: MOCK.courses[i]
    }));
    ...
    const schData = await tryFetch('/api/schedules/student/' + (currentStudent ? currentStudent.id : 0));
    schedules = (schData && schData.length) ? schData : MOCK.schedules;
...
function loadAreaPersonal() {
    ...
    const enr = enrollments.length ? enrollments : MOCK.enrollments.map((e, i) => ({
        ...e,
        course: MOCK.courses[i]
    }));
    ...
    <div class="personal-card">
      ...
      <div class="personal-card-body">
        ...
        return `<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;margin-bottom:3px"><span style="font-size:12.5px">${g.course ? g.course.name : '—'}</span><strong style="font-size:13px;color:${cl === 'high' ? 'var(--success)' : cl === 'mid' ? 'var(--gold)' : 'var(--error)'}">${v.toFixed(1)}</strong></div><div class="grade-bar"><div class="grade-fill ${cl}" style="width:${(v / 10) * 100}%"></div></div></div>`;
    }).join('')}
      </div>
    </div>
    ...
    <div class="personal-card">
      ...
      <div class="personal-card-body">
        ...
        return `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid rgba(11,31,58,0.05);font-size:13px"><span style="font-weight:500">${c.name || 'Curso'}</span><span style="color:var(--text-muted);font-size:12px">${d}</span></div>`;
    }).join('')}
      </div>
    </div>
    ...
    <div class="personal-card">
      ...
      <div class="personal-card-body">
        ...
        return `<div style="padding:10px;background:var(--cream);border-radius:8px;margin-bottom:8px"><div style="font-weight:600;font-size:13px;margin-bottom:8px">${c.name || 'Curso'} — ${t}</div><div style="display:flex;gap:4px" id="stars-${i}">${[1, 2, 3, 4, 5].map(n => `<button onclick="setRating(${i},${n})" style="background:none;border:none;cursor:pointer;padding:2px"><svg width="20" height="20" fill="none" stroke="var(--gold)" stroke-width="2" viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg></button>`).join('')}</div></div>`;
    }).join('')}
...
function submitEvals() {
    showToast('Evaluaciones enviadas. ¡Gracias por tu opinión!', 'success');
}
...
function openCourseView(course) {
    ...
    // Show course view, hide main content panels
    ...
    document.getElementById('breadcrumbCourseName').textContent = currentCourse.name || 'Curso';
    const teacherEl = document.getElementById('cvTeacherName');
    if (teacherEl) teacherEl.textContent = currentCourse.teacher && currentCourse.teacher.user ? currentCourse.teacher.user.name : '';
...
function renderUnitTabs() {
    ...
    if (!units.length) {
        bar.innerHTML = '<div style="padding:0 20px;font-size:13px;color:var(--text-muted);display:flex;align-items:center;min-height:52px">Sin unidades registradas</div>';
        return;
    }
...
// ─── Render Unit Content (writes into #unitContentArea) ──────────────────────
function renderUnit(idx) {
    ...
    <div class="card" style="margin-bottom:20px">
      ...
      <div class="card-body">
        ${announcements.map(a => `
        <div class="announcement-card open" style="margin-bottom:10px">
          <div class="announcement-card-header" style="cursor:default">
            <div class="announcement-card-icon"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg></div>
            <div class="announcement-card-meta"><div class="announcement-card-title">${a}</div></div>
          </div>
        </div>`).join('')}
      </div>
    </div>` : ''}
    ...
    <div class="card" style="margin-bottom:20px">
      ...
      <div class="card-body">
          ...
          <div class="activity-card-header">
            ...
            <div class="activity-header-meta">
              ...
              <div class="activity-due-row">
                ${a.dueDate ? `<span class="activity-due"><svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/></svg>Entrega: ${new Date(a.dueDate + 'T00:00:00').toLocaleDateString('es-CO', {
        day: 'numeric',
        month: 'long',
        year: 'numeric'
    })}</span>` : ''}
        ...
        </div>`).join('') : '<div style="color:var(--text-muted);font-size:13px;padding:4px 0">Sin talleres asignados para esta unidad.</div>'}
      </div>
    </div>
    ...
    <div class="card" style="margin-bottom:20px">
      ...
      <div class="card-body">
        ...
        <div class="exam-card" style="margin-bottom:12px">
          <div class="exam-card-header">
            ...
            <div class="exam-header-meta">
              ...
              ${x.examDate ? `<div class="exam-date"><svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/></svg>Fecha: ${new Date(x.examDate + 'T00:00:00').toLocaleDateString('es-CO', {
        weekday: 'long',
        day: 'numeric',
        month: 'long',
        year: 'numeric'
        ...
        </div>`).join('') : '<div style="color:var(--text-muted);font-size:13px;padding:4px 0">Sin evaluaciones programadas para esta unidad.</div>'}
      </div>
    </div>

    ${forums.length ? `
    <div class="card" style="margin-bottom:20px">
      <div class="card-header"><span class="card-title">Foros de Discusión</span><span class="badge badge-teal">${forums.length}</span></div>
      <div class="card-body">
        ${forums.map(f => `
        <div class="forum-card" style="margin-bottom:12px">
          <div class="forum-card-header">
            ...
            <div class="forum-card-meta"><div class="forum-card-title">${f.title}</div><div class="forum-card-desc">${f.description || ''}</div></div>
          </div>
        </div>`).join('')}
      </div>
    </div>` : ''}
    ...
    ${resources.length ? `
    <div class="card" style="margin-bottom:20px">
      <div class="card-header"><span class="card-title">Bibliografía y Recursos</span><span class="badge badge-success">${resources.length}</span></div>
      <div class="card-body">
        ${resources.map(r => `
        <a class="resource-card-item" href="${r.url || '#'}" target="_blank">
          <div class="resource-icon ${r.type || 'doc'}"><svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">${r.type === 'video' ? '<polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2"/>' : '<path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/>'}</svg></div>
...
document.getElementById('logoutBtn').addEventListener('click', () => {
    ...
    localStorage.removeItem('educat_email');
    sessionStorage.removeItem('educat_auth');
    sessionStorage.removeItem('educat_email');
